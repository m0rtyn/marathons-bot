...
import { addParticipantToSheet, checkUser, getNextChapter, setChapterAsRead } from "./spreadsheet.js"
...
export async function onStart(ctx: Context) {
  ...
  if (isLoggedIn) {
    ...
    ctx.reply('Would you like to join a marathon? (WIP)', Markup.keyboard([
...
export async function logUserIn(ctx: Context) {
  ...
  if (isLoggedIn) {
    ...
    ctx.reply(`Now, you are logged in the marathon. \nYou can check it at ${SS_URL}`)
  }
}
